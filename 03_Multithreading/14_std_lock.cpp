...
void transferMoney(BankAccount &source, BankAccount &destination, int amount){
    ...
    unique_lock<mutex> sourceLock(source.getMutex(), defer_lock);
    unique_lock<mutex> destinationLock(destination.getMutex(), defer_lock);
    cout << "Waiting to acquire both account mutexes" << endl;
    lock(sourceLock, destinationLock);
